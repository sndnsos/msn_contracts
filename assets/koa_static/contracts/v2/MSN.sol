...
contract MSN is ERC20 {
    ...
    constructor(string memory  name ,string memory symbol, uint256 inisupply) ERC20(name, symbol) {
    ...
    function add_special(address special_addr,string calldata _name) external onlyContractOwner {
    ...
    function remove_special(address special_addr) external onlyContractOwner {
        require(bytes(special_list[special_addr]).length!=0, "No such special");
    ...
    function get_special(address special_addr) external view returns (string memory){
        require(bytes(special_list[special_addr]).length!=0, "No such special");      
